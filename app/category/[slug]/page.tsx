...
export default function CategoryPage() {
  ...
  const activeFiltersCount = (selectedMaterial !== 'All' ? 1 : 0) + (selectedPriceRange.label !== 'All' ? 1 : 0)
    ...
    <main className="min-h-screen bg-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
        ...
        <p className="text-gray-600 max-w-2xl">
          ...
          Each piece is crafted with precision and care to bring you timeless elegance.
        </p>
      </div>
                ...
                <div className="space-y-2">
                  {priceRanges.map((range) => (
                    <label key={range.label} className="flex items-center cursor-pointer group">
                      ...
                      />
                      <span className={`ml-3 text-sm group-hover:text-gray-900 ${
                        selectedPriceRange.label === range.label ? 'text-gray-900 font-medium' : 'text-gray-600'
                      }`}>
                        {range.label}
                      </span>
                    </label>
